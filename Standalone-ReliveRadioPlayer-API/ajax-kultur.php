...
if (isset($_GET["callback"]) && !empty($_GET["callback"])) {
   ...
   ####### Anzahl API zum Auslesen der Liste
	if (isset($_GET["anzahl"]) && !empty($_GET["anzahl"])) {
		$anzahl = $_GET["anzahl"];
	} else { echo "Error: Ajax - Kulturliste!"; }

  	$anzahlouts = ($anzahl - 1);
  
  ########## OUTs //START ############
  	echo $callback. "('";
			echo '<tr class="chaptertr active" data-start="0" data-end="9938" data-img="http://static.reliveradio.de/logos/' .$suche["live_episode"]["db"]["slugintern"]. '.jpg"><td class="starttime"><span>00:00:00</span></td><td class="chapterimage"><img src="http://static.reliveradio.de/logos/' .$suche["live_episode"]["db"]["slugintern"]. '.jpg"></td>		<td class="chaptername"><span>'.preg_replace("/(')+/","&rsquo;",$suche["live_episode"]["db"]["name"]). ' - '.preg_replace("/(')+/","&rsquo;",$suche["live_episode"]["track_title"]). ' <a href="'.$suche["live_episode"]["db"]["url"].'"></a></td><td class="timecode"><span>'. substr($suche["live_episode"]["ends"], 11, -6) .'</span></td></tr>';
		for($i = 0; $i < $anzahlouts; ++$i) {
			echo '<tr class="chaptertr" data-start="0" data-end="9938" data-img="http://static.reliveradio.de/logos/' .$suche["upcoming_episodes"][$i]["db"]["slugintern"]. '.jpg"><td class="starttime"><span>'. date("H:i:s", (strtotime(substr($suche["upcoming_episodes"][$i]["starts"], 11,-6))-strtotime($server))) .'</span></td><td class="chapterimage"><img src="http://static.reliveradio.de/logos/' .$suche["upcoming_episodes"][$i]["db"]["slugintern"]. '.jpg"><td class="chaptername"><span>'.preg_replace("/(')+/","&rsquo;",$suche["upcoming_episodes"][$i]["db"]["name"]). ' - '.preg_replace("/(')+/","&rsquo;",$suche["upcoming_episodes"][$i]["track_title"]). ' <a href="'.$suche["upcoming_episodes"][$i]["db"]["url"].'"></a></td><td class="timecode"><span>'. substr($suche["upcoming_episodes"][$i]["ends"], 11, -6) .'</span></td></tr>';
		}
	echo "')";
  ########## OUTs //END #############
}
